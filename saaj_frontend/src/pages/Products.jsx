...
import { SEO, ItemListJsonLd, BreadcrumbJsonLd } from "@/components/SEO";
...
const ProductsPage = () => {
  ...
  const seoCategory = selectedCategory === "All" ? "" : ` ${selectedCategory}`;
  const seoTitle = selectedCategory === "All"
    ? "Shop All Jewellery Collections"
    : `${selectedCategory} Collection — Shop Online`;
  const seoDescription = selectedCategory === "All"
    ? "Explore the full SAAJJEWELS collection — necklaces, bracelets, earrings, jhumkas, rings, gift hampers and more. Premium quality at pocket-friendly prices."
    : `Shop beautiful ${selectedCategory} jewellery online at SAAJJEWELS. Handcrafted with love, delivered across India.`;
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-secondary/30">
      ...
      {filteredProducts.length > 0 && (
        <ItemListJsonLd
          ...
          listName={`SAAJJEWELS${seoCategory} Collection`}
        />
      )}
            ...
            {filteredProducts.map((product, index) => (
              ...
              >
                <div className="aspect-square overflow-hidden relative bg-gradient-to-br from-rose-50 to-amber-50">
                  <img
                    src={product.image}
                    alt={`${product.name} - Buy ${product.category || "Jewellery"} Online at SAAJJEWELS`}
                    loading="lazy"
