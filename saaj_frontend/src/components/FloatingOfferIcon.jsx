...
export default function FloatingOfferIcon() {
  ...
  useEffect(() => {
    const fetchPopup = async () => {
      try {
        ...
        if (data && data.id) setPopupConfig(data);
      } catch {
        setPopupConfig({
          title: "Claim 10% OFF",
          couponCode: "SAAJ10",
  ...
  const handleClaim = async (e) => {
    ...
    if (!lower.endsWith("@gmail.com")) { setError("Please provide a Gmail address (example@gmail.com)."); return; }
  ...
  const tooltipLabel = popupConfig?.couponDiscountType === "fixed"
    ? `Get ₹${popupConfig.couponDiscountValue} OFF`
    : `Get ${popupConfig?.couponDiscountValue || 10}% OFF`;
          ...
          <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
            <div className="bg-[#071226] text-[#e6e1d9] px-4 py-2 rounded-lg shadow-xl text-sm font-medium">
              <div className="text-center">
                <div className="font-bold text-[#c6a856]">{tooltipLabel}</div>
                {remainingOffers > 0 && (
                  <div className="text-xs text-[#cbd5e1] mt-1">{remainingOffers} customers left!</div>
                )}
              </div>
            ...
            {!claimed ? (
              <form onSubmit={handleClaim} className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button type="submit" className="flex-1 bg-gradient-to-r from-[#c6a856] to-[#f4e4bc] text-[#062132] py-2 rounded-lg text-sm font-medium hover:from-[#d4b56e] hover:to-[#f8ecd0] transition-all duration-200">
                    Get Coupon
                  </button>
                  <button type="button" onClick={() => setShowModal(false)} className="px-3 py-2 bg-transparent text-[#c6a856] rounded-lg text-sm font-medium border border-[#c6a856]/30 hover:bg-[#0b1420]/40 transition-all duration-200">
                    Cancel
                  </button>
                </div>
              </form>
              ...
              <div className="text-center space-y-3">
                ...
                <button
                  onClick={() => { setShowModal(false); setShowIcon(false); localStorage.removeItem("saaj_jewels_offer_declined"); }}
